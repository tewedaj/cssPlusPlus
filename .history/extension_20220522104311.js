...
//Update  React Native inline CSS with Name
	...
	//code to use latter
		...
		vscode.window.activeTextEditor.edit((editBuilder) => {
			editBuilder.replace(new vscode.Range(0,0,reactJs.pageLine,0),"import {styles} from ./"+folderUri.path.split("/")[folderUri.path.split("/").length-1]+"/"+folderUri.path.split("/")[folderUri.path.split("/").length]+ +"/styles/styles.js; \n" +reactJs.pageContent);
		});
